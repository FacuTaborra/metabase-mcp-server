...
class AiohttpMetabaseGateway:
    ...
    async def _request(
        ...
        try:
            ...
            try:
                # On any error status, surface the actual Metabase response body
                # (not just the HTTP reason) so 4xx/5xx are diagnosable.
                if response.status >= 400:
                    error_text = await response.text()
                    logger.error(f"HTTP {response.status} on {endpoint}: {error_text[:500]}")
                    raise MetabaseResponseError(
                        response.status,
                        error_text[:500] or response.reason,
                        endpoint,
                    )
        ...
        except MetabaseResponseError:
            # Already shaped with the response body — don't rewrap.
            raise
